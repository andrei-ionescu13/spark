...
import { Box, Card, FormHelperText, Grid, TextField, Typography } from '@mui/material';
...
export const DiscountForm: FC<DiscountFormProps> = (props) => {
  ...
  const updateDiscount = useUpdateDiscount(() => queryClient.invalidateQueries(['discount', discount?._id]))
  ...
  const onShouldSetEndDateChange = (event: ChangeEvent<HTMLInputElement>): void => {
        ...
        onSubmit={(values, { resetForm }) => {
          ...
          if (mode === 'create') {
            createDiscount.mutate(formValues, {
              onSuccess: (discount) => {
                resetForm();
                toast.success(ToastItemCreated('discount', `/discounts/${discount._id}`));
              },
              onError: (error) => {
          ...
          if (discount) {
            updateDiscount.mutate({ id: discount._id, body: formValues }, {
              onSuccess: async () => {
